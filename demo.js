...
function renderStandingsFoTile() {
  ...
  el.innerHTML = `
    <div class="bg-aew-card rounded-xl border border-gray-800 p-4">
      <h3 class="font-bold text-aew-gold mb-1">Foreign Objects</h3>
      <p class="text-xs text-gray-500 mb-3">Awarded to each region's last-place team (by standings). Owner picks one; if Wheel Of Boom is chosen, the commissioner spins the wheel.</p>
      <div class="space-y-2">
        ${fos.map(fo => `
...
function setTxTab(which) {
  ...
  if (which === 'trades') renderTradeRulesPublic();
}
