...
function renderStandings() {
  const list = (div) => Object.entries(data.users)
    ...
    .map(([pin, u]) => ({ pin, name: u.name, points: data.points[pin] || 0, maxRoster: getTeamMaxRoster(pin), lastDelta: u.lastDelta }))
    ...
    el.innerHTML = arr.map((t, i) => {
      ...
      return `<div class="standing-row ${t.pin === currentUser ? 'me' : ''}">
        <div class="flex items-center gap-3">
          ...
          ${t.maxRoster > getBaseRosterSize() ? `<span class="text-xs bg-emerald-900 text-emerald-300 px-1.5 py-0.5 rounded">${t.maxRoster}</span>` : ''}
        </div>
...
function lastPlaceInDivision(div) {
  const teams = Object.entries(data.users)
    .filter(([_, u]) => u.division === div && !u.isCommissioner)
...
function sameDivPartners(pin) {
  ...
  return Object.entries(data.users)
    .filter(([p, u]) => p !== pin && u.division === me.division && !u.isCommissioner)
...
function renderClaimList() {
  ...
  el.innerHTML = claimRanked.map((w, i) => `
    <div class="flex items-center justify-between gap-2 py-1.5 text-sm">
      ...
      <button type="button" class="claim-remove text-red-400 text-xs flex-shrink-0" data-i="${i}">✕</button>
    </div>`).join('');
  el.querySelectorAll('.claim-remove').forEach(b => {
    b.onclick = () => { claimRanked.splice(+b.dataset.i, 1); renderWaiver(); };
  });
}
...
function renderDropList() {
    ...
    <div class="space-y-1 mb-2">
      ${dropRanked.length ? dropRanked.map((w, i) => `
        <div class="flex items-center justify-between gap-2 text-sm py-1">
          <span class="text-gray-500 text-xs w-5">${i + 1}.</span>
          <span class="flex-1 truncate">${escapeHtml(w)}</span>
          <button type="button" class="drop-remove text-red-400 text-xs" data-i="${i}">✕</button>
  ...
  el.querySelectorAll('.drop-remove').forEach(b => {
    b.onclick = () => { dropRanked.splice(+b.dataset.i, 1); renderDropList(); };
  });
  ...
  el.innerHTML = legend + teams.map(([pin, u]) => {
    ...
    return `<div class="bg-aew-card rounded-xl border border-gray-800 p-4">
      <div class="flex justify-between mb-2">
        <span class="font-semibold">${escapeHtml(u.name)}${pin === currentUser ? ' (you)' : ''}${champCrown(pin)}</span>
...
function collectCalendarFromDom() {
  ...
  if (!list) return (data.calendar || []).map(e => ({ ...e }));
  const events = (data.calendar || []).map(e => ({ ...e }));
  list.querySelectorAll('[data-i]').forEach(block => {
    const i = parseInt(block.getAttribute('data-i'), 10);
    if (isNaN(i) || !events[i]) return;
    // Only read fields from the expanded editor row for this index
    const nameEl = block.querySelector('.cal-name');
    if (!nameEl) return; // collapsed row — keep existing data
    events[i] = {
      name: (nameEl.value || '').trim() || events[i].name,
      type: block.querySelector('.cal-type')?.value || events[i].type || 'Weekly TV',
      howOften: block.querySelector('.cal-often')?.value || events[i].howOften || 'Weekly',
      day: block.querySelector('.cal-day')?.value || '',
      date: block.querySelector('.cal-date')?.value || '',
      notes: (block.querySelector('.cal-notes')?.value || '').trim()
    };
  });
  return events;
}
...
function openCalendarEdit(i) {
  ...
  // Flush any open editor fields first
  data.calendar = collectCalendarFromDom();
  calendarEditIndex = idx;
  renderCalendarEditor();
  // Focus name field so it's obvious edit mode opened
...
function bindCalendarEditorClicks() {
  ...
  // Event delegation — survives full innerHTML re-renders
...
function renderCalendarEditor() {
  ...
  el.innerHTML = events.map((s, i) => {
    ...
    const when = s.howOften === 'Weekly' && s.day ? `Every ${s.day}` : (s.date || s.howOften || '');
    if (!editing) {
      return `
    ...
    return `
      <div class="bg-black/40 rounded-lg p-3 border border-aew-gold/60 space-y-2 ring-1 ring-aew-gold/30" data-i="${i}">
        ...
        <p class="text-[11px] text-gray-500">Editing — change fields, then <strong>Done</strong>. Click <strong>Save Calendar</strong> to keep changes permanently.</p>
      </div>`;
  }).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('calendar-save')?.addEventListener('click', () => {
    const events = collectCalendarFromDom().filter(e => e.name && e.name.trim());
    calendarEditIndex = null;
    saveCalendar(events);
