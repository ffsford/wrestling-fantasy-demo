...
function loadWheelConfig() {
  ...
  return { enabled: true, segments: DEFAULT_WHEEL.map(s => ({ ...s })) };
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('wheel-enabled')?.addEventListener('change', (e) => {
    ...
    saveWheelConfig({ enabled: data.wheelEnabled, segments: data.wheel || DEFAULT_WHEEL });
    renderWheelEditor();
  });
  ...
  document.getElementById('wheel-save')?.addEventListener('click', () => {
    ...
    data.wheel = segs;
    saveWheelConfig({ enabled: data.wheelEnabled, segments: segs });
    drawWheel();
    const msg = document.getElementById('wheel-editor-msg');
