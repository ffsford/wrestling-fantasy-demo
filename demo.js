...
const DEFAULT_SCHEDULE = [
  { name: 'AEW Dynamite', type: 'Weekly TV', howOften: 'Weekly', day: 'Wednesday', nextDate: '', notes: '' },
  { name: 'AEW Collision', type: 'Weekly TV', howOften: 'Weekly', day: 'Saturday', nextDate: '', notes: '' },
  { name: 'WWE Raw', type: 'Weekly TV', howOften: 'Weekly', day: 'Monday', nextDate: '', notes: '' },
  { name: 'WWE SmackDown', type: 'Weekly TV', howOften: 'Weekly', day: 'Friday', nextDate: '', notes: '' }
];
const SHOW_TYPES = ['Weekly TV', 'Special TV', 'PPV/PLE'];
...
const SCHEDULE_KEY = 'wf_demo_schedule';
...
function renderScheduleView() {
  ...
  if (!shows.length) {
    el.innerHTML = '<p class="text-gray-500 text-sm py-8 text-center">No shows scheduled yet. Commissioner can add them under Commissioner tools.</p>';
    return;
  }
  // Group by type
  const order = ['Weekly TV', 'Special TV', 'PPV/PLE'];
  const groups = {};
  shows.forEach(s => {
    const t = s.type || 'Weekly TV';
    if (!groups[t]) groups[t] = [];
    groups[t].push(s);
  });
  el.innerHTML = order.filter(t => groups[t]?.length).map(type => `
    <div class="mb-6">
      <h3 class="text-sm font-semibold text-gray-400 mb-2 uppercase tracking-wide">${type}</h3>
      <div class="space-y-2">
        ${groups[type].map(s => `
          <div class="bg-aew-card rounded-xl border border-gray-800 p-4 flex flex-wrap items-start justify-between gap-2">
            <div>
              ...
              <div class="text-sm text-gray-400 mt-0.5">
                ${s.howOften === 'Weekly' && s.day ? `Every ${escapeHtml(s.day)}` : escapeHtml(s.howOften)}
                ${s.nextDate ? ` · Next: ${escapeHtml(s.nextDate)}` : ''}
              </div>
...
function renderScheduleEditor() {
  const el = document.getElementById('schedule-editor-list');
  if (!el) return;
  const shows = data.schedule || [];
  el.innerHTML = shows.map((s, i) => `
    <div class="bg-black/40 rounded-lg p-3 border border-gray-800 space-y-2" data-i="${i}">
      <div class="flex flex-wrap items-center gap-2">
        <input type="text" class="sch-name flex-1 min-w-[10rem] bg-black border border-gray-700 rounded px-2 py-1.5 text-sm font-semibold" value="${escapeHtml(s.name)}" placeholder="Show name (e.g. Dynamite, Raw, SmackDown)" />
        <button type="button" class="sch-del text-red-400 text-xs hover:text-red-300 px-1" data-i="${i}">✕</button>
      </div>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div>
          <label class="block text-[10px] text-gray-500 mb-0.5">Type</label>
          <select class="sch-type w-full bg-black border border-gray-700 rounded px-2 py-1.5 text-sm">
            ${SHOW_TYPES.map(t => `<option value="${t}" ${s.type === t ? 'selected' : ''}>${t}</option>`).join('')}
          </select>
        </div>
        <div>
          <label class="block text-[10px] text-gray-500 mb-0.5">How often</label>
          <select class="sch-often w-full bg-black border border-gray-700 rounded px-2 py-1.5 text-sm">
        ...
        <div>
          ...
          <select class="sch-day w-full bg-black border border-gray-700 rounded px-2 py-1.5 text-sm">
            <option value="">—</option>
            ${DAYS.map(d => `<option value="${d}" ${s.day === d ? 'selected' : ''}>${d}</option>`).join('')}
          </select>
        </div>
        <div>
          <label class="block text-[10px] text-gray-500 mb-0.5">Next date</label>
          <input type="date" class="sch-date w-full bg-black border border-gray-700 rounded px-2 py-1.5 text-sm" value="${escapeHtml(s.nextDate || '')}" />
        </div>
      </div>
      <input type="text" class="sch-notes w-full bg-black border border-gray-700 rounded px-2 py-1.5 text-xs text-gray-300" value="${escapeHtml(s.notes || '')}" placeholder="Notes (optional)" />
    </div>
  `).join('') || '<p class="text-gray-500 text-sm">No shows yet. Click + Add show.</p>';
  el.querySelectorAll('.sch-del').forEach(btn => {
    btn.onclick = () => {
      data.schedule.splice(+btn.dataset.i, 1);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('schedule-add')?.addEventListener('click', () => {
    ...
    data.schedule.push({ name: 'New show', type: 'Weekly TV', howOften: 'Weekly', day: 'Wednesday', nextDate: '', notes: '' });
    renderScheduleEditor();
  });
  document.getElementById('schedule-reset')?.addEventListener('click', () => {
    data.schedule = DEFAULT_SCHEDULE.map(s => ({ ...s }));
    saveSchedule(data.schedule);
    renderScheduleEditor();
    if (activeTab === 'schedule') renderScheduleView();
  ...
  document.getElementById('schedule-save')?.addEventListener('click', () => {
    ...
    list.querySelectorAll('[data-i]').forEach(row => {
      ...
      shows.push({
        name,
        type: row.querySelector('.sch-type')?.value || 'Weekly TV',
        howOften: row.querySelector('.sch-often')?.value || 'Weekly',
        day: row.querySelector('.sch-day')?.value || '',
        nextDate: row.querySelector('.sch-date')?.value || '',
        notes: row.querySelector('.sch-notes')?.value?.trim() || ''
      });
    });
    saveSchedule(shows);
    const msg = document.getElementById('schedule-msg');
    if (msg) {
      msg.textContent = shows.length ? `Saved ${shows.length} show(s)` : 'Schedule cleared';
      msg.className = 'text-sm mt-2 text-emerald-400';
    }
    renderScheduleEditor();
    if (activeTab === 'schedule') renderScheduleView();
